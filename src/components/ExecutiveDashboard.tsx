...
export const ExecutiveDashboard: React.FC<ExecutiveDashboardProps> = ({
  ...
  // Chart 3 Data: Diamond Carat Run-Rate vs Stock
  const caratVelocityData = useMemo(() => {
  ...
  // Top 5 and Lowest 5 Carat / Weights (Best-Selling vs Lowest-Selling)
  ...
  return (
    <div className="space-y-5" id="necessary-dashboard-root">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-4 sm:p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-xs text-slate-500 dark:text-slate-400 mt-1.5">
            ...
            <span>•</span>
            <span>0.02 ct Solitaire Base Rule</span>
            <span>•</span>
      ...
      {/* Secondary Chart Row: Carat Demand & Aging Profile */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Chart C: Diamond Carat Run-Rate vs Available Stock */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 sm:p-5">
          <div className="flex items-center justify-between mb-3">
            <div>
              <h4 className="text-xs sm:text-sm font-bold text-slate-900 flex items-center gap-1.5">
                <Gem className="w-4 h-4 text-indigo-500" />
                Carat Velocity: Stock vs Sales Run-Rate
              </h4>
              <p className="text-[11px] text-slate-500">Fastest selling diamond carats vs available pieces</p>
            </div>
          </div>

          <div className="h-44 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={caratVelocityData} margin={{ top: 5, right: 10, left: -25, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" vertical={false} />
                <XAxis dataKey="carat" tick={{ fontSize: 10, fill: '#64748b' }} axisLine={false} tickLine={false} />
              ...
              <div className="space-y-1">
                {top5Weights.map((w, idx) => (
                  <div
                    ...
                    className="py-1 px-2 rounded bg-slate-900/80 hover:bg-slate-800 border border-slate-800/60 transition-all cursor-pointer flex items-center justify-between gap-1.5 group text-[11px]"
                    title={`Click to filter by ${w.weight} ct`}
                  >
                    <div className="flex items-center gap-1.5 min-w-0">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden transition-all">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                  Product Velocity: High Demand vs Stagnant Carats
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-3">
                ...
                >
                  ...
                  <p className="text-[11px] text-emerald-800">
                    {report.consignmentRecommendations?.length || 0} high-demand diamond carats are depleted in company stock.
                  </p>
                </div>
