  ...
  const handleApplySingleHtml = () => {
    ...
    try {
      ...
      if (res.records.length > 0) {
        onImportRecords(res.records, report.period);
        setStatusMsg({
          type: 'success',
          text: `Successfully ingested ${res.records.length} records (${res.summary.totalSold} sold, ${res.summary.totalStock} stock)!`
        });
      } else {
  ...
  const handleApplyDualHtml = () => {
    ...
    try {
      ...
      if (mergedRecords.length > 0) {
        onImportRecords(mergedRecords, report.period);
        setStatusMsg({
          type: 'success',
          text: `Combined ${mergedRecords.length} branch/weight records successfully!`
        });
      } else {
            ...
            <div className="space-y-5">
              <div className="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-2">
                <div className="flex items-center gap-2 text-xs font-bold text-cyan-400">
                  ...
                  <span>Oracle Reports HTML Compatibility Guaranteed</span>
                </div>
                <p className="text-xs text-slate-400 leading-relaxed">
                  The parser recognizes exact Oracle Reports HTML structures for <strong className="text-white">Diamond Ct. Wise Sold</strong> and <strong className="text-white">Diamond Ct. Wise Stock</strong> across all 20 branch stores.
                </p>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  <label className="text-xs text-slate-300 block font-medium">
                    1. Diamond Ct. Wise Sold (HTML):
                  </label>
                  <textarea
                    rows={3}
                    placeholder="Paste Diamond Ct. Wise Sold HTML here..."
                    value={soldHtmlText}
                ...
                <div className="space-y-2">
                  <label className="text-xs text-slate-300 block font-medium">
                    2. Diamond Ct. Wise Stock (HTML):
                  </label>
                  <textarea
                    rows={3}
                    placeholder="Paste Diamond Ct. Wise Stock HTML here..."
                    value={stockHtmlText}
