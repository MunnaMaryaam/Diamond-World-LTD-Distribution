...
export const DataImportModal: React.FC<DataImportModalProps> = ({
  ...
  const [periodText, setPeriodText] = useState(currentPeriod || "01-03-2026 to 19-09-2026");
  const [quickPeriod, setQuickPeriod] = useState<string>('custom');
  ...
  const handlePeriodQuickChange = (preset: string) => {
    ...
    if (preset === '1m') {
      ...
      setPeriodText(`01-01-${today.getFullYear()} to ${formatD(today)}`);
    }
  };
  ...
  const handleApply = () => {
    ...
    onImportRecords(parsedResult.records, periodText.trim() || currentPeriod);
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/75 backdrop-blur-sm animate-fade-in">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl max-w-3xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="p-5 sm:p-6 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="bg-slate-850 border border-slate-750 p-3.5 rounded-xl space-y-2">
            <div className="flex items-center justify-between flex-wrap gap-2">
              ...
              <div className="flex items-center gap-1 text-[11px]">
                <button
                  type="button"
                  onClick={() => handlePeriodQuickChange('1m')}
                  className={`px-2 py-0.5 rounded ${quickPeriod === '1m' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
                >
                  Last 1 Month
                </button>
                <button
                  type="button"
                  onClick={() => handlePeriodQuickChange('3m')}
                  className={`px-2 py-0.5 rounded ${quickPeriod === '3m' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
                >
                  Last 3 Months
                </button>
                <button
                  type="button"
                  onClick={() => handlePeriodQuickChange('6m')}
                  className={`px-2 py-0.5 rounded ${quickPeriod === '6m' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
                >
                  Last 6 Months
                </button>
                <button
                  type="button"
                  onClick={() => handlePeriodQuickChange('custom')}
                  className={`px-2 py-0.5 rounded ${quickPeriod === 'custom' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}
                >
                  Custom
                </button>
              </div>
            ...
            <p className="text-[11px] text-slate-400">
              This period will be stamped on all executive movement summaries, Excel exports, and procurement requisition sheets.
            </p>
          </div>
