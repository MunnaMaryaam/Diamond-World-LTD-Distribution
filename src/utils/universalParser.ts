...
export function mergeDualStockAndSoldData(
  ...
  stockItems.forEach(item => {
    const key = `${item.branch}__${item.weight}`;
    if (!map[key]) {
      map[key] = { branch: item.branch, weight: item.weight, soldQty: 0, currentStock: 0 };
    }
  ...
  return {
    ...
    detectedFormat: 'EXCEL',
    summary: computeSummary(mergedRecords)
  };
}
...
function normalizeWeight(val: string): string {
  if (!val) return 'Item-1';
  const str = String(val).trim();
  const cleaned = str.replace(/ct|carat|gm|pcs/gi, '').trim();
  // If it is purely a decimal/integer number (e.g. "0.24", "1.5", "2")
  ...
  // Otherwise it's a dynamic product name, shade, size, or SKU code (e.g. "Hydra Glow Serum 30ml", "Slim Oxford - M", "Pro 256GB")
  return str;
}
...
function normalizeRecordList(items: any[]): RawInventoryRecord[] {
  return items.map((item, i) => ({
    ...
    branch: item.branch || item.Branch || item.store || item.Store || item.location || 'Main Branch',
    weight: normalizeWeight(String(item.weight || item.Weight || item.carat || item.Carat || item.particular || item.Particular || item.sku || item.SKU || item.item || item.Item || item.product || item.Product || `Item-${i + 1}`)),
